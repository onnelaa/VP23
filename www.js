...
http.createServer(function(req, res){
	if (req.method === 'POST'){
		collectRequestData(req, result => {
			...
			fs.open('public/log.txt', 'a', (err, file)=>{
				if(err){
					...
					fs.appendFile('public/log.txt', result.nameInput + ',' + result.lastNameInput + ';', (err)=>{
						if(err){
							throw err;
		...
		if(currentURL.pathname === "/"){
			...
			res.write('\n\t <p>TLÜ <a href="tluphotos">foto</a>.</p>');
			res.write(pageFoot);
		...
		else if (currentURL.pathname === "/tluphotos"){ 
			...
			//res.write('\n\t<img src="tlu_2.jpg" alt="TLÜ foto">');
			//res.write('\n\t <p><a href="/">Tagasi avalehele</a>!</p>');
			//res.write(pageFoot);
			//et see kõik valmiks ja ära saadetaks
			//return res.end();
		...
		//else if (currentURL.pathname === "/tlu_2.jpg"){
		else if (path.extname(currentURL.pathname) === '.jpg'){
			console.log(path.extname(currentURL.pathname));
			//console.log("tahan pilti");
			//let filePath = path.join(__dirname, "public", "tluphotos/tlu_2.jpg");
